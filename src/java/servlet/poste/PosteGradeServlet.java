 ...
 * @author chalman
 */
@WebServlet(name = "PosteGradeServlet", urlPatterns = {"/PosteGrade"})
public class PosteGradeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet PosteGradeServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet PosteGradeServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            PosteGrade posteGrade = new PosteGrade();
            if(type.equals("1")) {
                String poste = request.getParameter("poste");
                String grade = request.getParameter("grade");
                String tauxHoraire = request.getParameter("tauxHoraire");
                
                posteGrade = new PosteGrade(poste, grade, tauxHoraire);
            } 
            else if(type.equals("2")) {
                String poste = request.getParameter("poste");
                String grade = request.getParameter("grade");
                String posteRelier = request.getParameter("posteRelier");
                String gradeRelier = request.getParameter("gradeRelier");
                String coefficient = request.getParameter("coefficient");
                
                posteGrade = new PosteGrade(poste, grade, posteRelier, gradeRelier, coefficient);
            } else {
                throw new Exception("Erreur : le type hidden type est null");
            }
            
            GenericDAO.save(posteGrade, null);
